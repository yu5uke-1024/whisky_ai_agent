...
2. 抽出情報の確認・修正
   - 抽出結果をユーザーに提示し、修正要望があればmodify_fieldツールで該当フィールドを修正します。
...
2. whisky_label_processorで新規抽出
   ↓
3. 必要に応じてmodify_fieldで修正
   ↓
4. 最新情報をユーザーに提示・保存の確認
   * ウイスキー情報の内容: {whisky_info?}
   ↓
5. 保存依頼があればsave_whisky_info_to_firestoreで保存
   ↓
6. 保存後、テイスティングノート作成の希望を確認し、必要ならtasting_note_agentへ依頼
